...
scenario('Front end crop', ({ step, before }) => {
  let adminPage;
  let visitorPage;
  let postId;
  let cropId;
  ...
  before(async () => {
    ...
    // The visitor has no cookies, so the form uses the basic uploader
    visitorPage = await browser.newPage({ isolated: true });
  });

  step('creates a field group with a 16:9 crop field', async () => {
    await wpAdmin.createCropFieldGroup(adminPage, { width: 16, height: 9 });
  });
  ...
  step(
    'uploads and crops an image from the front end as a visitor',
    async () => {
  ...
  step('saves the cropped image as the field value', async () => {
